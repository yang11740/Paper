...
class CsvDataset(Dataset):
    ...
    def __getitem__(self, idx):
        # idx = 5
        # The CSV stores 'input_images/0000X.jpg', but the files are in 'dataset/synthetic_chinese_handwriting/input_images'
        img_path = str(self.images[idx])
        if img_path.startswith('input_images/'):
            parent_dir = os.path.dirname(self.gt_dir.rstrip('\\/'))
            img_path = os.path.join(parent_dir, img_path)
            
        img = np.array(Image.open(img_path).convert('RGB'))
        
        # Mask files are PNG, while images may be JPG
        mask_filename = str(self.images[idx]).split('/')[-1]
        mask_filename = os.path.splitext(mask_filename)[0] + '.png'
        gt = np.array(Image.open(os.path.join(self.gt_dir, mask_filename)))
